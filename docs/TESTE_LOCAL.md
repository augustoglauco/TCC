...
| --- | --- | --- |
| `vendas` | Integração do Orquestrador com o catálogo (R12, Fase 5): estoque, cotação com e sem desconto por volume, compatibilidade sim/não, produto inexistente, domínio que não é vendas, duas conversas de acompanhamento em que a 2ª mensagem não cita o produto (V8, V9) e uma consulta genérica por categoria sem citar modelo (V10, "tem geradores no estoque?", bug de 2026-09-26) | `vendas_catalogo_consulta` (campo `resultado`: `sem_termos`, `sem_candidatos`, `llm_sem_produto`, `produto_inexistente`, `ok`, `ok_categoria` ou `categoria_vazia`, mais `termos`, `termos_historico`, `candidatos`, `slots` e o `bloco` injetado no prompt), `vendas_catalogo_consulta_falhou`, `rag_indisponivel` |
