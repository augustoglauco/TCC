...
| Admin — Modelos locais (Ollama) | `/admin/modelos` | Página interna (acessível pelo menu de engrenagem ⚙️ no cabeçalho e pelo rodapé) para listar/ativar em runtime/baixar (Ollama ou Hugging Face GGUF) modelos locais de chat, além de ajustar em runtime a temperatura do modelo local, os timeouts local/externo, a flag de fallback de domínio do RAG e o provedor do classificador de intenção do roteador — heurística + LLM local (padrão) ou TypeSafe Jev via OpenRouter (seção "Parâmetros de execução") — ferramenta de teste, não substitui a escolha de produção da Fase 10 — decisão registrada em `docs/ARCHITECTURE.md` §5 | Não (`# MVP: sem autenticação, ver docs/ARCHITECTURE.md §5`) |
