...
| Admin — Catálogo de Produtos | `/admin/produtos` | Página interna (acessível pelo menu de engrenagem ⚙️) para cadastro manual de produtos, custos de fornecedor, preços de venda, fotos vetorizadas no CLIP, e assistente Human-in-the-Loop de importação de catálogos (PDF multipáginas e múltiplas fotos) | Não (`# MVP: sem autenticação`) |
