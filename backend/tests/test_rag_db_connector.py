...
async def test_read_table_as_text_le_todas_as_colunas_por_reflexao(db_session):
    ...
    textos = await read_table_as_text(connection, "produtos")
...
async def test_read_table_as_text_restringe_colunas_informadas(db_session):
    ...
    await connection.execute(
        text("INSERT INTO produtos (nome, preco) VALUES ('Gerador Diesel GD-15', 24900.00)")
    )
...
async def test_read_table_as_text_tabela_vazia_retorna_lista_vazia(db_session):
    connection = await db_session.connection()
    await connection.execute(text("CREATE TABLE produtos (id INTEGER PRIMARY KEY, nome TEXT)"))

    assert await read_table_as_text(connection, "produtos") == []
...
async def test_leitura_da_tabela_e_ingestao_no_rag_criam_um_documento_por_linha(
    ...
    connection = await db_session.connection()
    await connection.execute(
        text("CREATE TABLE produtos (id INTEGER PRIMARY KEY, nome TEXT, descricao TEXT)")
    )
    await connection.execute(
        text(
